...
\begin{answer}
运行成功。一次实测中 block 的输出顺序为 2、1、3、0；每个 block 内观察到
thread 0--7 依次输出。block 之间的先后顺序不是程序语义的一部分，多次运行
可以变化。
\end{answer}
...
\begin{answer}
\texttt{02\_device\_query.cu} 当前五个字段仍未填写，因此该文件不能编译。
独立查询到的硬件信息如下，后续实验也使用这块卡：
\begin{center}\small
\begin{tabular}{@{}ll@{}}
\toprule
项目 & 实测值 \\
\midrule
型号 / compute capability & RTX 4060 Laptop GPU / 8.9 \\
SM 数量 & 24 \\
warp 大小 & 32 \\
shared memory / block & 48 KB（默认上限） \\
最大常驻线程 / SM & 1536 \\
显存总量 & 8188 MiB \\
\bottomrule
\end{tabular}
...
\begin{answer}
(a) 错。TFLOPS 描述大量并行运算的总吞吐，不代表单条指令延迟。\\
(b) 对。标称带宽依赖连续、合并且并发度足够高的访问。\\
(c) 对。严格的数据依赖链无法利用 GPU 的大规模并行吞吐。\\
(d) 错。1000 TFLOPS 是每秒总运算次数，不是一次运算的延迟。
\end{answer}
...
\begin{answer}
峰值 FLOPS 依赖大量独立运算同时在途。严格在线算法的下一步必须等待上一步，
无法铺开足够并行度，只能逐次承担单步指令和访存延迟，因此总时间由依赖链长度
乘以单步延迟决定，而不是由峰值吞吐决定。
\end{answer}
...
执行层次 & 软件含义 & 对应硬件 & 直接可用的存储 & 同步与通信手段 \\
\midrule
warp & 32 个线程的执行组 & warp scheduler 与 32 个 lane & 各线程寄存器 & warp primitive、shuffle、syncwarp \\
block / CTA & 可协作的一组线程 & 驻留在同一个 SM & 寄存器与 shared memory & syncthreads、原子操作 \\
grid & 一次 kernel 的全部 block & 多个 SM & global / constant memory & 分 kernel 同步；特定条件下 cooperative groups \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
SIMD 由一条显式向量指令操作多个数据元素；SIMT 暴露多个标量线程，硬件把
线程组成 warp 共同取指和发射。判断为错：独立 program counter 改善了线程级
调度和同步语义，但同一 warp 走不同分支时仍需按 active mask 执行各条路径，
divergence 的串行化代价仍然存在。
\end{answer}
...
\begin{answer}
硬件：RTX 4060 Laptop GPU（compute capability 8.9）。
\begin{center}\small
\begin{tabular}{@{}lll@{}}
\toprule
配置 & 耗时 (ms) & ns / 元素 \\
\midrule
CPU 单线程 & 7.361 & 1.76 \\
GPU \texttt{<<<1,1>>>} & 377.797 & 90.07 \\
GPU \texttt{<<<1,256>>>} & 8.106 & 1.93 \\
GPU 铺满 grid & 1.330 & 0.32 \\
\bottomrule
\end{tabular}
\end{center}
(a) GPU 单线程既承担较高的设备端指令/访存延迟，又没有利用并行执行资源，
因此明显慢于 CPU。 (b) 从单 block 到铺满 grid 的提速说明 GPU 加速依赖大量
并发 warp 隐藏延迟并聚合吞吐，而不是依赖单线程速度。
\end{answer}
...
\begin{answer}
已完成。判测结果：\texttt{5 passed}。实现按 active lane mask 递归执行
\texttt{if\_lt} 的两条路径，无 active lane 的分支不计 cycle，汇合后恢复
当前层 mask。
\end{answer}
...
\begin{answer}
当前源码强制重编译成功，运行结果为 \texttt{PASS}。实现使用
\texttt{blockIdx.x * blockDim.x + threadIdx.x} 计算全局下标，以
\texttt{idx < n} 保护尾部，并按 256 threads/block 向上取整 grid。
\end{answer}
...
\begin{answer}
(a) \texttt{\_\_global\_\_}；(b) \texttt{\_\_device\_\_}；
(c) \texttt{\_\_host\_\_ \_\_device\_\_}；
(d) \texttt{\_\_constant\_\_}；(e) \texttt{\_\_shared\_\_}。
\end{answer}
...
\begin{answer}
Unified Memory 版本运行 \texttt{PASS}，本次“搬运 + kernel + 读回”耗时
112.4 ms。(a) kernel launch 对 host 异步；CPU 读取结果前必须确保 GPU
写入结束。显式内存版本中，device-to-host 的同步
\texttt{cudaMemcpy} 已经承担了这个同步点。(b) 当前仓库未保留改造前的实测
基线，不能给出可信的两版倍率。Unified Memory 的计时包含 CPU/GPU 间按页
迁移及缺页处理，表现受 GPU、互连和页迁移策略影响。
\end{answer}
...
\begin{answer}
(a) 错，kernel launch 通常异步返回。\\
(b) 对，同一 stream 保持提交顺序，同步的 D2H copy 会等待前序 kernel。\\
(c) 错，非法访存通常在后续同步或错误检查 API 处暴露。
\end{answer}
...
\begin{answer}
当前仍未修复，运行结果为 \texttt{FAIL}，首个元素为
\texttt{got 0, want 6.73}。launch 使用 2048 threads/block，超过设备的
1024 threads/block 上限，因此 kernel 没有执行。若不在 launch 后检查错误，
异步启动错误会一直到后续 CUDA API 才被观察到，原程序因而看起来“一声不吭”。
\end{answer}
...
\begin{answer}
当前运行结果为 \texttt{PASS}，二维行列索引和边界保护已生效。不过 x 方向
grid 表达式当前等价于分配 700 个 block，而不是期望的约 44 个 block；
结果正确但启动了大量不做工作的 block，建议继续修正向上取整括号。
\end{answer}
...
\begin{answer}
运行结果为 \texttt{PASS}。grid-stride loop 让 launch 配置与问题规模解耦，
同一 kernel 能处理任意 $n$，也便于限制 block 数和复用线程。这里
$n=2^{24}$，16384 个线程平均各处理 1024 个元素；代价是并发线程较少、每个
线程循环更久，可能降低延迟隐藏能力并增加循环控制开销。
\end{answer}
...
\begin{answer}
一次实测的 block 顺序为
11、5、14、8、2、4、10、13、7、1、15、9、12、6、3、0。
(a) 顺序由 GPU 的 block scheduler、资源可用情况和运行时状态决定。
(b) 正确性不能依赖 block 执行顺序。block 可按任意顺序映射到任意数量的 SM，
正是同一程序能够随 GPU 规模伸缩的基础。
\end{answer}
...
\begin{answer}
判测脚本覆盖的
$n\in\{0,1,31,1024,1025,2^{20},2^{20}+3\}$ 全部通过，对应 SUM 分别为
0、$-1536$、$-46686$、$-525312$、$-525824$、$-1048576$、$-1053178$。
\end{answer}
...
\begin{answer}
(a) 线性编号为 $5\times 8+3=43$，位于 warp 1、lane 11（均从 0
开始编号）。(b) 64 个线程占 2 个 warp。(c) 33 个线程占 2 个 warp，
第二个 warp 只有 1 个有效线程，浪费 31 个 lane 的执行容量。
\end{answer}
...
\begin{answer}
预测按 warp 对齐版本约快 2 倍。实测：warp 内分支 1.591 ms，按 warp
分支 0.801 ms，比值 1.99。奇偶分支使每个 warp 依次执行两条路径；按 warp
划分时每个 warp 只执行一条。若两支工作量不同，divergent warp 的时间近似为
两支路径时间之和；warp 对齐版本中单个 warp 只承担一支，整个 kernel 的完成
时间取决于两类 warp 的工作量、数量和调度，通常受较重路径影响。
\end{answer}
...
\begin{answer}
保留同步时运行 \texttt{PASS}。(a) 写 shared memory 和交叉读取之间缺少
block 级 barrier 时，某些线程会在对应写入完成前读取，形成数据竞争。
(b) $t$ 与 $255-t$ 不可能位于同一 warp；正确位置只来自当次 warp 调度恰好
让生产者先完成写入，属于未定义的时序巧合，不能依赖。
\end{answer}
...
\begin{answer}
标准做法是结束当前 kernel，在 host 侧以同步点或同一 stream 的顺序保证其
完成，再启动下一个 kernel；kernel 边界天然提供全 grid 同步。特殊场景可用
cooperative launch 配合 cooperative groups 的 grid sync，但要求硬件和
驻留条件满足限制。
\end{answer}
...
\begin{answer}
两个版本均 \texttt{PASS}。交错配对平均 0.1080 ms，连续配对平均
0.0714 ms，前者是后者的 1.51 倍。交错配对让活跃 lane 在 warp 内稀疏分布，
同时每轮含取模和更不规则的控制；连续配对把活跃线程集中在低编号 lane，
分支执行和地址模式更规整，因此更快。
\end{answer}
...
空间 & 谁可见 & 生命周期 & 片上 / 片外 & 谁管理 \\
\midrule
local & 单个线程 & 线程 & 片外显存（经 cache） & 编译器 \\
shared & 同一 block & block & 片上 & 用户声明，硬件分配 \\
global & 全 device / host 经 API & 分配到释放 & 片外 & 用户与 runtime \\
constant & 全 device 只读 & module / context & 片外，片上缓存 & 用户与 runtime \\
L1 / L2 cache & L1 偏 SM，L2 全 device & 硬件管理 & 片上 & 硬件 \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
静态 shared memory 和动态 shared memory 两个版本均通过，输出为
\texttt{static PASS}、\texttt{dynamic PASS}、\texttt{PASS}。每个 block
加载 256 个主体元素和左右各 1 个 halo，同步后从 shared tile 计算三点平均。
\end{answer}
...
\begin{answer}
两版均 \texttt{PASS}：global 1.0457 ms，constant 1.0567 ms，
\texttt{global / constant = 0.99x}，没有观测到提速。constant cache 的核心
优势是一个 warp 中多个线程读取同一个地址时可广播一次结果，并能缓存小型只读
数据；本题只有 8 个系数，global 版本也很容易被普通 cache 命中，故差距很小。
\end{answer}
...
\begin{answer}
(a) 对，local 表示线程私有地址空间，不表示物理位置在片上。\\
(b) 对，编译器无法把运行期动态索引完全映射到寄存器时，数组可能 spill 或
直接放入 local memory。
\end{answer}
...
\begin{answer}
运行结果为 \texttt{PASS}，平均 6.9916 ms，吞吐 2.40 GB/s。实现使用
\texttt{atomicAdd} 更新全局的 256 个 bucket，保证并发计数不丢失。
\end{answer}
...
\begin{answer}
两版均 \texttt{PASS}。naive 为 6.9802 ms、2.40 GB/s；shared 私有化版为
0.1206 ms、139.07 GB/s，提速 57.86 倍。提速来自把绝大多数高冲突的原子更新
分散到每个 block 的片上 shared histogram，最后每个 block 只对 256 个全局
bucket 做一次合并，显著减少全局原子竞争和显存往返。
\end{answer}
...
stride & 1 & 2 & 4 & 8 & 16 & 32 \\
\midrule
GB/s & 128.3 & 88.9 & 52.2 & 28.3 & 28.1 & 28.4 \\
\bottomrule
\end{tabular}
\end{center}
stride 增大后，同一 warp 的相邻线程访问越来越分散，一个 memory transaction
中被实际使用的数据比例下降，coalescing 变差，因此有效带宽快速下降；到
stride 8 以后基本落到相近的平台。
\end{answer}
...
\begin{answer}
本机是 RTX 4060 Laptop GPU：100 KB shared memory/SM，1536
threads/SM。
\begin{center}\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
...
occupancy & 100\% & 100\% & 100\% & 83.3\% & 50.0\% & 16.7\% \\
实测带宽 (GB/s) & 129.0 & 127.5 & 128.0 & 123.7 & 130.6 & 81.7 \\
\bottomrule
\end{tabular}
\end{center}
(a) 以 55 KB 为例，shared memory 只容纳 1 个 block；每 block 256
threads，所以 occupancy 为 $256/1536=16.7\%$，与 API 一致。
(b) 常驻 warp 越少，某个 warp 等待显存时可切换执行的其他 warp 越少，
内存延迟更难被隐藏，带宽会在并发度不足时下降。
(c) 本机实际档位不是题干示例的 75\%、37.5\%、12.5\%，且数据并非严格单调：
100\% 到 83.3\% 从 129.0 降至 123.7 GB/s，约下降 4.1\%；50\% 到
16.7\% 从 130.6 降至 81.7 GB/s，约下降 37.4\%。高 occupancy 区间已有
足够 warp 覆盖延迟，减少少量并发影响不大；降到 1 block/SM 后在途请求不足，
带宽才明显受损。
\end{answer}
...
\begin{answer}
实测：host 不同步 0.0169 ms，host 同步 1.9389 ms，cudaEvent
0.4035 ms。(a) 应使用 cudaEvent 的 0.4035 ms 作为 device 时间线上的 kernel
耗时。(b) host 不同步只测到 launch 提交及 host 侧调用开销；host 同步测到
从 launch 到 device 完成的 wall-clock 时间，还包含 host 调用、同步及调度
开销，因此不如 cudaEvent 纯粹。
\end{answer}
...
\begin{answer}
(a) 对；(b) 对；(c) 对。前两项来自 stream 顺序和异步 launch 语义；
Unified Memory 在 CPU/GPU 间维护页驻留和一致性，CPU 访问 GPU 驻留页可能
触发 fault、等待和页迁移。
\end{answer}
...
\begin{answer}
(a) 错，tile 是带形状和布局语义的值/视图，不等同于可直接修改的显存区域。
(b) 对。(c) 错，同一程序可以混合不同 kernel 编程模型，底层仍由 CUDA
执行和调度。
\end{answer}
...
\toprule
 & CUDA SIMT & cuTile & Triton \\
\midrule
并行单位 & block 里的 thread & block & program instance / block \\
编号 & \texttt{blockIdx} / \texttt{threadIdx} & \texttt{ct.bid} & \texttt{tl.program\_id} \\
数据分工 & 线程用全局下标划分 & 用户定 tile，编译器分配线程 & program 定 tile，编译器分配 lane \\
边界处理 & \texttt{if} 判断 & tiled view 的 load/store 处理 & 用户提供 load/store mask \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
(a) 由编译器根据 tile 形状和目标硬件决定。(b) 代码中没有显式出现
\texttt{threadIdx}、\texttt{blockDim}、线程全局下标、每线程边界分支、
线程到元素映射、合并访存细节以及 block 内同步。
\end{answer}
...
\begin{answer}
已完成，GPU 判测 \texttt{3 passed}，覆盖 4096、10000 和 3 个元素。
kernel 使用 \texttt{program\_id} 定位 program tile，以
\texttt{tl.arange} 生成 offsets，并用 mask 处理非整块尾部。
\end{answer}
...
\begin{answer}
当前未完成：原有 \texttt{scale} 测试通过，但 fused 的两组参数测试失败，
总结果为 \texttt{1 passed, 2 failed}。kernel 内已经写出
\texttt{relu(a*x+b)}，但 wrapper 仍固定传 \texttt{a=1,b=0}。
这类改动应集中在 tile 上的逐元素表达式和参数传递；program 到 tile 的映射、
offset、mask 和 load/store 主体与具体逐元素公式无关，所以无需重写。
\end{answer}
...
\begin{answer}
已完成，\texttt{scale\_add} 判测通过。二维 CTA grid 分别对 $N$、$M$
按 tile 大小向上取整，block 内用 \texttt{T.Parallel} 遍历 tile，并对
ragged 边界做条件保护。
\end{answer}
...
\begin{answer}
已完成，4 组形状
\texttt{(64,128)、(123,77)、(1,500)、(500,1)} 全部通过。grid 尺寸仍由
用户通过 \texttt{T.ceildiv} 给出，行列位置变成当前 CTA tile 的二维起点；
逐元素边界保护不再显式出现，由 \texttt{T.copy} 对越界 tile 自动处理。
\end{answer}
...
谁负责 & CUDA SIMT & cuTile & Triton & TileLang \\
\midrule
线程到数据的映射 & 用户 & 编译器 & 编译器；用户定 program tile & 编译器；用户定 CTA tile \\
边界处理 & 用户 & 编译器 & 用户提供 mask & \texttt{T.copy} 可由编译器处理；显式循环由用户处理 \\
tile / block 尺寸的选择 & 用户 & 用户 & 用户 & 用户 \\
block 内同步 & 用户 & 编译器 & 编译器 & 编译器按 copy/gemm/pipeline 插入 \\
Tensor Core 指令选择与供数布局 & 用户（显式 MMA） & 编译器 & 编译器；用户指定精度/形状 & 编译器；本次 sm\_90a 选 WGMMA，sm\_100a 选 MMA.SYNC \\
\bottomrule
\end{tabular}
\end{center}
\end{answer}
...
\begin{answer}
已完成，$256\times256\times128$ 的 fp16 输入、fp32 累加判测通过。
TileLang 版本显式分配 A/B shared tile、C fragment，并用
\texttt{T.Pipelined} 和 \texttt{T.gemm} 描述流水乘累加。Triton 暴露的是
program tile、load 和 \texttt{tl.dot} 等更高层操作；shared/register 放置、
同步和部分流水由编译器 lowering 根据布局与目标硬件选择，因此源码中没有逐项
声明。
\end{answer}
...
\begin{answer}
已完成，4 项 GPU 判测全部通过：$33\times127$、$8\times1000$、
$4\times256$ 的大数值稳定性、$5\times1$。实现按行启动 CTA，把行宽补到
不小于 $N$ 的 2 次幂，越界位置填 $-\infty$，依次执行稳定的 max、exp、
sum 归约，并按 $(M,N)$ 缓存编译结果；窄行用 128 threads，宽行用 256
threads。
\end{answer}
...
\begin{answer}
未完成。当前 \texttt{softmax(x)} 仍直接抛出
\texttt{NotImplementedError}，4 项测试全部失败。
\end{answer}
...
\begin{answer}
(a) 错，PTX 是虚拟 ISA，需要 driver JIT 成目标 GPU 的机器码。\\
(b) 错，只含 \texttt{sm\_70} SASS 不能跨 major architecture 直接在
compute capability 9.0 上执行。\\
(c) 对，一个 fatbin 可以同时包含多个 cubin/SASS 和 PTX fallback。\\
(d) 对，PTX 到目标 SASS 的 JIT 由 NVIDIA driver 在运行时完成。
\end{answer}
...
\begin{answer}
已完成本机实验，GPU 为 \texttt{sm\_89}。\\
(a) 只含 \texttt{sm\_90} SASS 的版本启动失败，报错：
\texttt{cudaErrorNoKernelImageForDevice: no kernel image is available for
execution on the device}，exit code 为 1。\\
(b) 只含 \texttt{compute\_75} PTX 的版本正常运行，exit code 为 0。
程序首次加载该 kernel 时，由 NVIDIA driver 的 JIT compiler 将 PTX 编译为
\texttt{sm\_89} 可执行机器码，结果通常会被缓存。
\end{answer}
...
\begin{answer}
Driver API 是较底层的 context、module、memory 和 kernel launch 接口，
通常以 \texttt{cu*} 命名；Runtime API 提供更高层的 C/C++ 编程体验，
负责隐式 context 初始化并与 \texttt{nvcc} 语言扩展配合，通常以
\texttt{cuda*} 命名。\texttt{cudaMalloc} 属于 Runtime API。
\end{answer}
...
\begin{answer}
\textbf{测试环境：}RTX 4060 Laptop GPU，compute capability 8.9，24 SM；
$M=N=K=2048$，Triton/TileLang 使用 fp16 输入、fp32 累加。

\textbf{Naive CUDA：} \texttt{BS=8/16/32} 的 \texttt{sm\_89} 版本均编译
成功，但运行时在第一次 \texttt{cudaMalloc} 处返回
\texttt{cudaErrorNoDevice}。当前环境中 PyTorch/Triton 可以访问 GPU，而独立
CUDA Runtime 进程不能枚举设备，因此没有记录不可信的 naive GFLOPS。

\textbf{Triton benchmark：}
\begin{center}\small
\begin{tabular}{@{}lrr@{}}
\toprule
配置 & 平均耗时 (ms) & TFLOPS \\
\midrule
\texttt{32x32x32} & 1.239 & 13.9 \\
\texttt{64x64x32} & 1.237 & 13.9 \\
\texttt{128x64x32} & 1.184 & 14.5 \\
\texttt{128x128x32} & 1.245 & 13.8 \\
\texttt{128x128x64} & 1.422 & 12.1 \\
\texttt{64x64x64} & 1.135 & 15.1 \\
\texttt{cuBLAS} & 1.080 & 15.9 \\
\bottomrule
\end{tabular}
\end{center}
正确性测试为 \texttt{2 passed}；该组配置中 Triton 最快的是
\texttt{64x64x64}。

\textbf{TileLang benchmark：}
\begin{center}\small
\begin{tabular}{@{}lrrr@{}}
\toprule
配置 & threads & 平均耗时 (ms) & TFLOPS \\
\midrule
\texttt{64x64x32, stages=1} & 128 & 1.204 & 14.3 \\
\texttt{64x64x32, stages=3} & 128 & 1.064 & 16.1 \\
\texttt{128x128x32, stages=3} & 128 & 1.116 & 15.4 \\
\texttt{128x128x64, stages=3} & 256 & 1.216 & 14.1 \\
\texttt{128x256x64, stages=1} & 256 & 1.609 & 10.7 \\
\bottomrule
\end{tabular}
\end{center}
TileLang matmul 正确性测试为 \texttt{1 passed}。原先
\texttt{128x256x64, stages=3} 需要 147456 B 动态 shared memory，超过本卡
100 KB/block 的限制，已改为 \texttt{stages=1} 后可运行。

tile 大小决定每个 program 的数据复用、global memory 访问形状、寄存器压力、
shared memory 用量和 occupancy；\texttt{BLOCK\_K} 或 \texttt{num\_stages}
增大不一定更快。流水深度为 3 的 \texttt{64x64x32} 在本次测量中隐藏了更多
搬运延迟，但较大的 \texttt{128x256x64} 又因 tile 过大而变慢。naive CUDA
每个线程只计算一个输出元素，沿 $K$ 维重复从 global memory 读取，缺少 tile
复用和 Tensor Core 路径，因此理论上会显著落后于两种 tiled 实现；本机尚未
取得它的可执行测量值。
\end{answer}

本次数字来自各自 \texttt{bench()} 的单次配置扫描，不能替代 profiler 分析；
结果也会随 GPU 时钟、驱动、CUDA/Triton/TileLang 版本变化。
\begin{lookback}
